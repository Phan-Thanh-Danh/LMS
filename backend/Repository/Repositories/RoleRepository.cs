    ...
    public class RoleRepository : IRoleRepository
    {
        ...
        public async Task<IActionResult> GetAllAsync()
        {
            var roles = await _context
                .VaiTros.Select(vt => new RoleResponse
                {
        ...
        public async Task<IActionResult> GetByIdAsync(int id)
        {
            var role = await _context
                .VaiTros.Where(vt => vt.MaVaiTro == id)
            ...
            if (role == null)
                return new NotFoundObjectResult(
                    new { success = false, message = "Vai trò không tồn tại." }
                );
        ...
        public async Task<IActionResult> CreateAsync(CreateRoleRequest request)
        {
            if (await _context.VaiTros.AnyAsync(vt => vt.TenVaiTro == request.TenVaiTro))
                return new ConflictObjectResult(
                    new { success = false, message = "Tên vai trò đã tồn tại." }
                );
            ...
            return new CreatedResult(
                $"/api/v1/roles/{newRole.MaVaiTro}",
                new
                {
                    success = true,
                    message = "Tạo vai trò thành công.",
                    data = new { newRole.MaVaiTro, newRole.TenVaiTro },
                }
            );
        }
        ...
        public async Task<IActionResult> UpdateAsync(int id, UpdateRoleRequest request)
        {
            ...
            {
                bool trungTen = await _context.VaiTros.AnyAsync(vt =>
                    vt.TenVaiTro == request.TenVaiTro && vt.MaVaiTro != id
                );
                if (trungTen)
                    return new ConflictObjectResult(
                        new { success = false, message = "Tên vai trò đã tồn tại." }
        ...
        public async Task<IActionResult> ToggleActiveStatusAsync(int id)
        {
            ...
            return new OkObjectResult(
                new { success = true, message = $"Vai trò đã được {trangThai}." }
            );
        }
        ...
        public async Task<IActionResult> HardDeleteAsync(int id)
        {
            ...
            if (Array.Exists(SystemRoleIds, sysId => sysId == id))
                return new BadRequestObjectResult(
                    ...
                    {
                        success = false,
                        message = "Không thể xóa các vai trò mặc định của hệ thống (ID 1-7).",
                    }
                );
            ...
            if (coNguoiDung)
                return new BadRequestObjectResult(
                    ...
                    {
                        success = false,
                        message = "Không thể xóa: vai trò này đang được gán cho người dùng. Hãy vô hiệu hóa thay vì xóa.",
                    }
                );
            ...
            return new OkObjectResult(
                new { success = true, message = "Đã xóa vĩnh viễn vai trò." }
            );
        }
    }
}
