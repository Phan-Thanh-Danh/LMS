...
namespace backend.Controllers
{
    ...
    public class PromotionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<CampaignResponse>> CreateCampaign([FromBody] CreateCampaignRequest request)
        {
            ...
            return CreatedAtAction(nameof(GetCampaign), new { id = campaign.MaChienDich }, new CampaignResponse
            {
        ...
        [HttpGet("coupons")]
        public async Task<ActionResult<IEnumerable<CouponResponse>>> GetCoupons([FromQuery] int? campaignId, [FromQuery] Guid? instructorId)
        {
        ...
        public async Task<ActionResult<CouponResponse>> GetCoupon(string code)
        {
            ...
            if (g == null) return NotFound(new { message = "Không tìm thấy mã giảm giá" });
        ...
        public async Task<ActionResult<CouponResponse>> CreateCoupon([FromBody] CreateCouponRequest request)
        {
            ...
            return CreatedAtAction(nameof(GetCoupon), new { code = coupon.MaGiamGia }, new { message = "Tạo mã giảm giá thành công", code = coupon.MaGiamGia });
        }

        [Authorize(Roles = "Admin,CMO,Instructor")]
        [HttpPut("coupons/{code}")]
        public async Task<IActionResult> UpdateCoupon(string code, [FromBody] CreateCouponRequest request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
        ...
        public async Task<IActionResult> ToggleCouponStatus(string code)
        {
            ...
            if (!success) return NotFound(new { message = "Không tìm thấy mã giảm giá" });
        ...
        public async Task<IActionResult> BulkGenerate([FromBody] BulkGenerateCouponRequest request)
        {
            ...
            for (int i = 0; i < request.Count; i++)
            {
                string code;
                do {
                    code = $"{request.Prefix}-{random.Next(1000, 9999)}-{Guid.NewGuid().ToString().Substring(0, 4)}".ToUpper();
                } while (!await _promotionRepository.IsCouponCodeUniqueAsync(code) || coupons.Any(c => c.MaGiamGia == code));

                coupons.Add(new GiamGia
                {
                    MaGiamGia = code,
                    MaChienDich = request.MaChienDich,
                    LoaiGiamGia = request.LoaiGiamGia,
        ...
        [HttpGet("validate")]
        public async Task<IActionResult> ValidateCoupon([FromQuery] string code, [FromQuery] decimal orderTotal)
        {
            var (isValid, message, discountAmount) = await _promotionRepository.ValidateCouponAsync(code.ToUpper(), orderTotal);
            if (!isValid) return BadRequest(new { message });

            return Ok(new { message, discountAmount, finalTotal = orderTotal - discountAmount });
        }
